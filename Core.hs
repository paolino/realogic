...
module Core -- (Rea (⋯), Serial (..), Recover, External, Internal, Reactor (⋯) , Response (⋯), makeReactor) where
	where
...
import Untypeds (Serial (Serial), Untyped (Untyped), casting)
...
stepRea :: (Monad m, Functor m) => Rea m -> Untyped -> Maybe (m ([Rea m],[Internal], Maybe (Rea m)))
...
purgeCoreDG ns (DepGraph _ resize _) = resize (let t = nub . concat . map (snd . contextualize) $ ns in  t)

----------------------------------------------------------------------------
type SerialReactor c = ([[Maybe Recover]],[SCore c])
